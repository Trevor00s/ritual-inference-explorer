...
export function startCritter(canvas, mouse, opts = {}) {
  ...
  const critter = setupLizard(8 / Math.sqrt(legNum), legNum, 16); // long, tapering tail
  ...
  function loop(t) {
    ...
    if (hunt) {
      var prey = hunt.nextOrdered(); // next pending tx to verify, in order
      if (prey) {
        gx = prey.x;
        gy = prey.y;
        var dx = critter.x - prey.x;
        var dy = critter.y - prey.y;
        if (dx * dx + dy * dy < 30 * 30) hunt.markEaten(prey.id); // contact → verify
      }
    }
