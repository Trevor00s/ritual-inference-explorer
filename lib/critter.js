...
// The Ritual mark: a point-up diamond subdivided into a lattice of small diamonds,
// stroked in the same neon line style as the creature body (no raster image).
function drawRitualMark(ctx, cx, cy, size, color) {
  const M = 4; // cells per side → MxM small diamonds
  const s = size / Math.SQRT2; // rotated-square side (square @45° reads as a point-up diamond)
  const step = s / M;
  ctx.save();
  ctx.translate(cx, cy);
...
export function startCritter(canvas, mouse, opts = {}) {
  const { color = "#19D184", hunt = null } = opts;
  ...
  function setupLizard(size, legs, tail) {
    var s = size;
    var critter = new Creature(W / 2, H / 2, 0, s * 18, s * 2, 0.5, 16, 0.5, 0.085, 0.5, 0.3);
    var spinal = critter;
    // Neck (shortened)
    for (var i = 0; i < 3; i++) {
  ...
  const legNum = 4; // quadruped lizard (clean + performant)
  const critter = setupLizard(8 / Math.sqrt(legNum), legNum, 12);
  ...
  function loop(t) {
    ...
    if (t - last < 16) return; // ~60fps (faster, smoother)
    last = t;
    ctx.clearRect(0, 0, W, H);

    // target: hunt nearest prey (failed/pending particle); else follow cursor
    var gx = mouse.x;
    var gy = mouse.y;
    if (hunt) {
      ...
      if (prey) {
        ...
        if (dx * dx + dy * dy < 26 * 26) hunt.markEaten(prey.id); // contact → eat
      }
    }
    ...
    // Ritual diamond-lattice mark as the head, drawn large in neon strokes (matches body)
    drawRitualMark(ctx, critter.x, critter.y, 46, color);
  }
  raf = requestAnimationFrame(loop);
