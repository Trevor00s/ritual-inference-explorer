...
export function startCritter(canvas, mouse, opts = {}) {
  ...
  // Logo-only entity: no skeleton lines render — the Ritual mark IS the whole creature.
  const s = 4;
  const critter = new Creature(W / 2, H / 2, 0, s * 12, s * 2, 0.5, 16, 0.5, 0.085, 0.5, 0.3);
  critter.draw = function () {}; // suppress body/leg/tail line drawing
  ...
  function loop(t) {
    ...
    // follow the transactions: chase nearest prey (failed/pending) and eat it;
    // otherwise drift toward the nearest tx particle. Never follows the cursor.
    var gx = W / 2;
    var gy = H / 2;
    if (hunt) {
      ...
      if (prey) {
        ...
        var any = hunt.nearestAny(critter.x, critter.y);
        if (any) {
          gx = any.x;
          gy = any.y;
        }
